# Anchor Core Pipeline — Technical Design

Reference for the **core engine**: verifiable extraction of logical units from a
text-extractable PDF as discrete, fully traceable text spans. A unit may be a
single contiguous span or an ordered set of **disjoint segments** (multi-span).
Each segment may carry a **role** so one logical unit can be split into
separate, linked outputs.

The engine is domain-agnostic. **Extraction profiles** (detection prompt +
optional unit-boundary regex + role conventions) specialize it. Compliance
frameworks (NIST AI RMF, HIPAA, …) are **example profiles**, documented in
[Part 3](#part-3--example-ai-rmf--compliance) and [profiles.md](profiles.md).
...
Long, structured PDFs (playbooks, standards, policies, specifications, contracts)
must be split into individual logical units, each linked back to its exact source
location. Two failure modes dominate naive LLM extraction:

- **Boundary errors.** Fixed page/token windows cut units in half and
  overlap-based stitching produces divergent duplicates.
- **Loss of provenance and fidelity.** When the model emits the unit body, it
  paraphrases, drops clauses, and cannot reliably report character offsets, so
  the output is not verifiable against the source.

A third structural need appears when a unit is **non-contiguous**: parts that
belong together are separated by prose the profile wants to exclude, yet they
must stay linked by the same record id. The AI RMF Playbook is the worked
example ([Part 3](#part-3--example-ai-rmf--compliance)).

### Approach: anchors, not bodies

The LLM never returns unit body text. For each unit it returns only **anchors**
— short verbatim substrings marking segment boundaries — plus an id, a status,
and (optionally) a per-segment `role`. Deterministic Python locates those
anchors in the source text, slices each segment, groups slices by role, and
emits typed outputs. The model does semantic work (what is a unit, where does
each part begin/end, what role each part plays); Python does everything
...
| Concern | Owner |
|---|---|
| Identify units, assign ids, choose segment anchors + roles, classify boundary status | **LLM** |
...
2. For every cleanly resolved **single-span** unit,
   `full_text[doc_offset_start:doc_offset_end] == original_text`.
3. For a **multi-span** unit the contiguous invariant does NOT hold;
...
[3] Unit-aware dynamic chunking           _compute_boundary_blocks, _build_batch
 |      (block-aligned batches sized to budget; never split a logical unit)
 v
[4] Anchor extraction (LLM)               extract_requirements_from_chunk
 |      (tool-use; per-unit segments[] with role + inclusive/exclusive anchors)
 v
[5] Segment resolution                     _resolve_chunk_anchors (global ordering)
 ...
 v
[6] Unit reconstruction                   extract_requirements_for_range, _stitch_chunk
...
profile declares a unit-boundary regex (`requirement_boundary_pattern`),
`_compute_boundary_blocks` marks the block indices where a new unit begins, and
`_build_batch` packs *whole units* — a single unit is never split across chunks.
Without a pattern it falls back to greedy block packing. See §5.
...
**[5] Segment resolution.** All segments of all units in a chunk are
...
units whose segments fall in different chunks via a single `pending`
...
| `role` | v0 enum: `requirement` (default) / `questionnaire` / `context` |
...
*Invariant:* for a single-span unit with `verbatim_match=True`,
...
A logical unit is an ordered list of segments (a single-span unit is just
...
together. Text **between** segments is intentionally excluded — that is the
point of multi-span: skip intervening prose while still capturing a later
segment linked by the same record id.

`original_text` is always the `requirement`-role text only (the primary
deliverable for that unit).

**v0 role enum.** `ANCHOR_INPUT_SCHEMA` fixes `role` to
`requirement` / `questionnaire` / `context`. This is a **profile convention**
used by bundled examples (especially AI RMF), not an open-ended taxonomy.
Extending the enum is future work. A profile that needs only one span should
emit a single `requirement`-role segment (or legacy top-level anchors).
...
complete final sentence, clause, or trailing history/citation bracket through
terminal punctuation. Exclusive `end_before_anchor` is used when the natural
boundary is the *start of the next labeled section/heading*; it need not be
globally unique — it resolves to the first occurrence after the segment start
within the ordering window.
...
all units in the chunk are flattened and resolved together in three passes
...
> `_resolve_anchors` (single-unit, no ordering context) is retained as a
...
resolved, the unit is defined by its segment document offsets. The pending
state persists resolved **offsets** (and the open segment spec), never re-scanning
prior chunks and never navigating backward.

### Multi-chunk units

If a unit's segments span chunks, the start chunk yields resolved
...
The key change over naive greedy chunking: a logical **unit** must never be
...
An example pattern (NIST AI RMF Playbook) is documented in
[Part 3](#part-3--example-ai-rmf--compliance)
...
units are handled by persisted state rather than re-reading, the full
...
A single `pending` slot threads an open unit across chunk boundaries. It
...
```
        ...
        PENDING (start offset + resolved segments + open spec)
            |                              \
            | continuation with resolved    \  new unrelated unit starts,
...
| **Interrupted (single-span)** | new unrelated unit starts before `pending` closes | Bound at the new start, `end_resolved=False`; process the new one |
...
segment of a multi-span unit is left open and a new unit then interrupts the
pending, stretching the last resolved segment to the interrupt point would
re-swallow the intentional inter-segment gap. So the stitcher emits only the
segments that actually resolved, marks `segments_partial=True` /
`end_resolved=False`, and moves on.
...
`_finalize_requirement` builds every document-level unit from resolved
...
wall time, token totals, per-batch `status_counts`, and unit quality
...
**Out of scope (reference implementation in the source application).** A parent
preprocessor repo may additionally emit Excel workbooks and feed an atomizer
...
| Case | Behavior |
|---|---|
| **Missing anchor** | The unresolved side yields a partial segment; the unit is recovered by a deterministic bound where possible and flagged `end_resolved=False`, or emitted empty if nothing resolves. Never dropped. |
| **Null end with immediate adjacency** | If the next unit start resolves in the same chunk, the stitcher closes at `next_start` and records `end_inferred_from_next_start=True`. |
...
| **Large unit (> 1 chunk)** | Stitched across chunks via `pending` (§6). Unit-aware chunking (§5) keeps a whole unit in one chunk when a `requirement_boundary_pattern` is set. |
...
| **Profile-specific rules** | The generic contract lives in `anchor_extract/prompts/anchor.txt` (loaded at import via `_load_anchor_system_prompt`; raises if missing/empty). The tool schema and public types remain requirement-centric in v0. A per-profile detection prompt decides only *what* is a unit and how ids are formatted; `build_anchor_system_prompt` composes the two and overrides any legacy "return body text" instruction. |
...
Authoring, composition via `build_anchor_system_prompt`, the checklist, and
bundled files: **[profiles.md](profiles.md)**.

The generic contract (`anchor_extract/prompts/anchor.txt`) is **not**
framework-agnostic in its public names: the tool is `emit_requirement_anchors`
and the payload key is `requirements`. It *is* domain-agnostic in behavior:
any profile that obeys the anchor rules can drive the same resolver and stitcher.
...
A Playbook subcategory interleaves the **normative outcome**, **explanatory
...
1. `role="requirement"` — subcategory id + one-sentence outcome (ends before
   "About").
2. `role="context"` — "About" through "Suggested Actions", ending before
   "Transparency & Documentation".
3. `role="questionnaire"` — questions only (exclusive start after
   "Organizations can document the following").

That mapping is a **profile convention**. The schema enum is still exactly those
three strings.

### Unit-boundary regex (AI RMF)
...
### Multi-span interrupt (same example)

If a later AI RMF segment is left open and a new subcategory interrupts the
pending unit, stretching the last resolved segment to the interrupt point would
re-swallow Suggested Actions / extended About / References. The stitcher emits
only resolved segments (`segments_partial=True`, `end_resolved=False`).
