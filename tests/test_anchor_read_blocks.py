...
class TestReadText(unittest.TestCase):
    ...
    def test_blocks_are_paragraphs(self):
        ...
        self.assertIsInstance(doc, DocumentExtraction)
        self.assertEqual(len(doc.blocks), 2)
        self.assertEqual(doc.blocks[0].text, "First para line one line two")
        self.assertEqual(doc.blocks[1].text, "Second para")
    ...
    def test_blocks_schema(self):
        payload = anchor.blocks(anchor.read(self.path))
    ...
    def test_include_full_text(self):
        doc = anchor.read(self.path)
        payload = anchor.blocks(doc, include_full_text=True)
        self.assertEqual(payload["full_text"], doc.full_text)

    def test_page_args_are_noop(self):
        full = anchor.read(self.path)
        paged = anchor.read(self.path, start_page=5, end_page=9)
        self.assertEqual(paged.full_text, full.full_text)
        self.assertEqual(paged.start_page, 1)
        self.assertEqual(paged.end_page, 1)
...
class TestReadPdf(unittest.TestCase):
    ...
    def test_matches_extract_pdf(self):
        ...
        self.assertEqual(via_read.full_text, direct.full_text)
